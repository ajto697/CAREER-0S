...
import { 
  ...
  GraduationCap, Award, Compass, BookOpen, Flame, Globe, MapPin
} from 'lucide-react';
...
export const IntroSequence: React.FC<Props> = ({ settings, onComplete }) => {
  ...
  const [stageVisualMode, setStageVisualMode] = useState<'pixel_art' | 'cinematic_video'>('pixel_art');
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-[#000] flex flex-col justify-between p-2 sm:p-4 md:p-6 overflow-hidden font-mono text-[#00ff41] select-none transition-all duration-500 ${isCrtFadingOut ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
      ...
      <div className="bg-black/95 border-b border-[#00ff41]/40 px-3 sm:px-6 py-2.5 flex items-center justify-between z-20 shrink-0">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex items-center gap-1.5 bg-[#0a180a] px-2.5 py-1 border border-[#00ff41]">
            ...
            <span className="text-[10px] sm:text-xs font-black text-white uppercase tracking-wider">
              CAREER-OS PROLOGUE 2026
            </span>
          </div>
          
          {/* Visual Mode Selector Buttons */}
          <div className="flex items-center gap-1 bg-black/80 p-0.5 border border-[#00ff41]/50">
            <button
              type="button"
              onClick={() => {
                playSound.click(soundEnabled);
                setStageVisualMode('pixel_art');
              }}
              className={`text-[10px] px-2 py-1 font-bold transition-all cursor-pointer flex items-center gap-1 ${
                stageVisualMode === 'pixel_art'
                  ? 'bg-[#00ff41] text-black border border-white shadow-[0_0_10px_#00ff41]'
                  : 'text-[#00ff41] hover:bg-[#00ff41]/20'
              }`}
            >
              <Globe className="w-3 h-3" />
              <span>🕹️ HOẠT HỌA PIXEL VIỆT NAM & TRÁI ĐẤT</span>
            </button>
            ...
            <button
              ...
              onClick={() => {
                playSound.click(soundEnabled);
                setStageVisualMode('cinematic_video');
              }}
              className={`text-[10px] px-2 py-1 font-bold transition-all cursor-pointer flex items-center gap-1 ${
                stageVisualMode === 'cinematic_video'
                  ? 'bg-[#00ffff] text-black border border-white shadow-[0_0_10px_#00ffff]'
                  : 'text-[#00ffff] hover:bg-[#00ffff]/20'
              }`}
            >
              <Film className="w-3 h-3" />
              <span>🎬 THƯỚC PHIM ĐIỆN ẢNH</span>
            </button>
          </div>
        </div>

        {/* Video Channels Switcher Pills (Visible only in video mode) */}
        {stageVisualMode === 'cinematic_video' && (
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="hidden sm:inline">{soundEnabled ? 'NHẠC NỀN ON' : 'TẮT ÂM'}</span>
          </button>
      ...
      <div className="relative flex-1 my-2 border-2 sm:border-4 border-[#00ff41] bg-black overflow-hidden flex flex-col justify-between shadow-[0_0_50px_rgba(0,255,65,0.25)]">
        ...
        {/* VIEW MODE 1: 8-BIT PIXEL ART VIETNAM & EARTH ANIMATED SCENE */}
        {stageVisualMode === 'pixel_art' ? (
          <div className="relative w-full h-full flex flex-col justify-between overflow-y-auto p-2 sm:p-4 z-5">
            {/* Upper Stage HUD Layer: Scene Tag */}
            <div className="relative z-10 p-2 sm:p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div className="space-y-1">
                ...
                <h1 className="text-base sm:text-xl md:text-2xl font-black text-white uppercase tracking-wider drop-shadow-[0_2px_10px_rgba(0,0,0,0.9)]">
                  {currentSceneData.title}
                </h1>
              </div>

              <div className="flex items-center gap-2 bg-black/90 px-2.5 py-1 border border-[#00ff41]/50 text-[10px] text-[#00ff41]">
                <span className="w-2 h-2 rounded-full bg-[#00ff41] animate-ping" />
                <span>MÔ PHỎNG ĐỊA LÝ & NGHỀ NGHIỆP 2026</span>
              </div>
            </div>

            {/* Pixel Vietnam & Earth Animated Scene Component */}
            <div className="my-1 sm:my-2 w-full max-w-5xl mx-auto">
              <PixelVietnamEarthScene enableSound={soundEnabled} />
            </div>

            {/* Subtitle Dialogue Container */}
            <div className="relative z-10 max-w-4xl mx-auto w-full pt-2">
              <div 
                onClick={handleNext}
                className="bg-[#050a05]/95 border-2 border-[#00ff41] p-3 sm:p-4 space-y-2.5 backdrop-blur-md shadow-[0_0_30px_rgba(0,0,0,0.8)] cursor-pointer hover:border-white transition-all group"
              >
                {/* Speaker Tag */}
                <div className="flex items-center justify-between border-b border-[#00ff41]/30 pb-1.5">
                  <div className="flex items-center gap-2">
                ...
                <p className="text-xs sm:text-sm text-white font-medium leading-relaxed tracking-wide min-h-[50px]">
                  {typedText}
                ...
                {currentSceneData.quote && (
                  <div className="bg-[#0c140c] border-l-4 border-[#ff00ff] p-2 sm:p-2.5 text-xs font-semibold text-[#00ff41] italic leading-snug">
                    {currentSceneData.quote}
                  </div>
                )}
                ...
                <div className="text-[10px] text-right text-[#00ff41]/70 group-hover:text-[#00ff41] pt-0.5">
          ...
          /* VIEW MODE 2: REAL HTML5 CINEMATIC VIDEO BACKGROUND */
          <div className="relative w-full h-full flex flex-col justify-between">
            ...
            {/* Lower Stage: Subtitle Dialogue Box */}
